...
class PageController extends Controller
{
    ...
    public function updatePasswordStore(UpdatePassword $request){
        ...
        return back()->withErrors(['old_password' => 'The old password does not match!'])->withInput();
    }

    public function wallet(){
        $userWallet = Auth::guard('web')->user();
        return view('frontend.wallet',compact('userWallet'));
    }

    public function transfer(){
        $userWallet = Auth::guard('web')->user();
        return view('frontend.transfer',compact('userWallet'));
    }
    ...
    public function transferConfirm(TransferFormValidate $request){
        ...
        $hash_value2 = hash_hmac('sha256',$str,'password123');
        ...
        if(!$from_account->wallet || !$to_account->wallet){
            return back()->withErrors(['fail' => 'Account user have no wallet yet!'])->withInput();
        }
    ...
    public function transferComplete(TransferFormValidate $request){
        ...
        try{
            ...
            // from_account_transaction_detail
            ...
            $message = 'Your wallet transfered ' . number_format($amount) .' MMK to '. $to_account->phone .' ('.$to_account->name.')' ;
            ...
            $message = 'Your wallet received ' . number_format($amount) .' MMK from '. $from_account->phone .' ('.$from_account->name.')' ;
    ...
    public function transferHash(Request $request){
        ...
        $hash_value = hash_hmac('sha256',$str,'password123');
